...
const newHeroCode = `      // Helper: Wait for an element to exist in the DOM
      ...
      function initializeHero() {
        ...
        var townName = '{{TOWN_NAME}}, Montana';
        ...
        img.onload = function() {
          ...
          waitForElement('#site-hero .hero-section').then(function(hero) {
            ...
            if (h1) h1.textContent = townName;
            if (p) p.textContent = 'A Montana Community';
            if (img) {
              img.src = townImg;
              img.alt = townName + ' - Scenic View';
            }
          }).catch(function() {
...
try {
  ...
  files.forEach(file => {
    ...
    try {
      ...
      // Replace the old hero initialization code
      const updatedContent = content.replace(
        /\/\/ Dynamically set hero image:.*?}\)\(\).*?<\/script>/s,
