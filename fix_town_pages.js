      ...
      function initializeHero() {
        ...
        img.onload = function() {
          ...
          waitForElement('#site-hero .hero-section').then(function(hero) {
            ...
            // Remove any duplicate h1 from the content section
            const contentH1 = document.querySelector('.content-section h1');
            if (contentH1 && contentH1.textContent.includes(townName)) {
              contentH1.remove();
            }
          }).catch(function() {
...
try {
  ...
  files.forEach(file => {
    ...
    try {
      ...
      // Remove the duplicate h1 from the content section
      content = content.replace(/<h1[^>]*>([^<]+), Montana<\/h1>\s*<h1>Quick Facts<\/h1>/i, '<h1>Quick Facts</h1>');
