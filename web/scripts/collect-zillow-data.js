...
// --- ZHVI (home values) ---
console.log('Processing Zillow ZHVI (home values)...');
const zhviText = fs.readFileSync(path.resolve(__dirname, 'zillow-zhvi-city.csv'), 'utf8');
...
// --- ZORI (rent) ---
console.log('\nProcessing Zillow ZORI (rent)...');
const zoriText = fs.readFileSync(path.resolve(__dirname, 'zillow-zori-city.csv'), 'utf8');
...
console.log(`Combined coverage: ${new Set([...ourSlugs.filter(s => housing[s]?.medianHomeValue), ...ourSlugs.filter(s => housing[s]?.zillowHomeValue)]).size}/134`);

// Sample comparisons
console.log('\n=== Sample Comparisons ===');
for (const s of ['bozeman', 'missoula', 'billings', 'whitefish', 'helena', 'great-falls', 'red-lodge', 'circle']) {
  const h = housing[s];
  if (!h) continue;
  const census = h.medianHomeValue ? `$${h.medianHomeValue.toLocaleString()}` : 'N/A';
  const zillow = h.zillowHomeValue ? `$${h.zillowHomeValue.toLocaleString()}` : 'N/A';
  console.log(`${s}: Census=${census} | Zillow=${zillow}`);
}
